...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  4 12:55:26 2020
...
## take KS 2sample test non significant genes
...
### load gscore name
##sampling = pd.read_csv('/Users/shanjuyeh/Desktop/Project/GeneExp_prediction/code/seperate_prediction/output/metabolite/LASSO.csv')
#sampling = pd.read_csv('/home/ubuntu/chenlab_deeplearning/chenlab_deeplearning_V2/DL_yeh/GeneExp_prediction/output/meta/other_models/LASSO.csv')
#sampling = sampling['Unnamed: 0']

## load pre-trained model
#json_file = open('/Users/shanjuyeh/Desktop/Project/GeneExp_prediction/code/encoder/encoder_ks10000_2step.json', 'r')
json_file = open('/home/ubuntu/chenlab_deeplearning/chenlab_deeplearning_V2/DL_yeh/GeneExp_prediction/encoder/encoder_nonTransfer_ks5000.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
# load weights into new model
#loaded_model.load_weights('/Users/shanjuyeh/Desktop/Project/GeneExp_prediction/code/encoder/encoder_ks10000_2step.h5')
loaded_model.load_weights('/home/ubuntu/chenlab_deeplearning/chenlab_deeplearning_V2/DL_yeh/GeneExp_prediction/encoder/encoder_nonTransfer_ks5000.h5')
...
### default intialization    
    ...
    new_model.add(LeakyReLU(alpha=0.1))
    new_model.add(Dense(units=384, input_dim=x_train.shape[1], activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
    new_model.add(Dropout(0.4))
    new_model.add(Dense(units=224,activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
    new_model.add(Dropout(0.4))
    new_model.add(Dense(units=32,activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
...
#name = 'metabolite_2-aminoadipate'
...
for method in methods:
    ...
    if method == 'default_init':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_default_meta,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_default_meta,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
    ...
    if method == 'PCA':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_pca_meta,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_pca_meta,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
