...
#### save scale up name
#meta_names_df = pd.DataFrame({'meta_name' : meta_names})
#meta_names_df.to_csv('/Users/shanjuyeh/Desktop/Project/GeneExp_prediction/code/metabolite/parallel/sacle_up/meta_name.csv')
...
def transfer_evaluate_model(x_train, y_train, x_test, y_test):
    ...
    history = History()
    new_model.fit(x_train, y_train, validation_split=0.1, epochs=100, batch_size=64, verbose=0, callbacks=[history, es], shuffle=True)
...
### default intialization    
    ...
    new_model.add(LeakyReLU(alpha=0.1))
    new_model.add(Dense(units=352, input_dim=x_train.shape[1], activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
    new_model.add(Dropout(0.55))
    new_model.add(Dense(units=320,activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
    new_model.add(Dropout(0.6))
    new_model.add(Dense(units=64,activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
