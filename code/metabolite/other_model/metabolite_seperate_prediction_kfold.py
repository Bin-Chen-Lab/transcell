...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 22 12:26:18 2020
...
def LASSO_FEATURE(exp_x, meta_name):
    y = meta_y[meta_name]
    lasso = Lasso(alpha=0.05, max_iter = 10000) # TUNED ## 44 features
    lasso.fit(exp_x, y)
...
def ELASTIC_FEATURE(exp_x, meta_name):
    y = meta_y[meta_name]
    elastic = ElasticNet(alpha=0.1, l1_ratio=0.3, max_iter = 10000) # TUNED ## 88 features
    elastic.fit(exp_x, y)
...
def ELASTIC(x_train, y_train, x_test, y_test):
    clf = ElasticNet(alpha=0.15, l1_ratio = 0.05, max_iter = 10000) # TUNED
    clf.fit(x_train, y_train)
    y_pred = clf.predict(x_test)
    mse = mean_squared_error(y_test, y_pred)
    pearsonr = scipy.stats.pearsonr(y_test, y_pred.flatten())
...
def RF_FEATURE(exp_x, meta_name):
    y = meta_y[meta_name]
    rf = RandomForestRegressor(n_jobs=-1, n_estimators= 500, oob_score = True, bootstrap = True, max_depth=80) # TUNED ## 2040 features
    rf.fit(exp_x, y)
...
for method in methods:
    ...
    if method == 'LASSO':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_elastic_meta,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_elastic_meta,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_rf_meta,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_rf_meta,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
