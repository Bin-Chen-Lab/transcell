...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  3 10:33:27 2020
...
### load gscore name
#sampling = pd.read_csv('/Users/shanjuyeh/Desktop/Project/GeneExp_prediction/output/CNV/other_models/LASSO.csv')
#sampling = sampling['Unnamed: 0']
##cn_names = sampling['Unnamed: 0']
...
def default_init_evaluate_model(x_train, y_train, x_test, y_test):
    ...
    new_model.add(Dense(units=256,activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
...
for method in methods:
    ...
    if method == 'default_init':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_default_cn,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_default_cn,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
    ...
    if method == 'PCA':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_pca_cn,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_pca_cn,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
