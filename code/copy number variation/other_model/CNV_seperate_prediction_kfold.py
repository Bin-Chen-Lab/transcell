...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 21 13:51:12 2020
...
def LASSO_FEATURE(exp_x, cn_name):
    y = cn_y[cn_name]
    lasso = Lasso(alpha=0.002, max_iter = 10000) # TUNED ## 862 features original:0.001, ks520:0.002
    lasso.fit(exp_x, y)
...
def LASSO(x_train, y_train, x_test, y_test):
    clf = Lasso(alpha=0.001, max_iter = 10000) # TUNED original:0.0001, ks520:0.001
...
def ELASTIC_FEATURE(exp_x, cn_name):
    y = cn_y[cn_name]
    elastic = ElasticNet(alpha=0.01, l1_ratio=0.3, max_iter = 10000) # TUNED ## 455 features
    elastic.fit(exp_x, y)
...
def ELASTIC(x_train, y_train, x_test, y_test):
    clf = ElasticNet(alpha=0.01, l1_ratio = 0.15, max_iter = 10000) # TUNED
    clf.fit(x_train, y_train)
    y_pred = clf.predict(x_test)
    mse = mean_squared_error(y_test, y_pred)
    pearsonr = scipy.stats.pearsonr(y_test, y_pred.flatten())
...
def RF_FEATURE(exp_x, cn_name):
    y = cn_y[cn_name]
    rf = RandomForestRegressor(n_jobs=-1, n_estimators= 500, oob_score = True, bootstrap = True, max_depth=80) # TUNED ## 679 features
    rf.fit(exp_x, y)
...
def RF(x_train, y_train, x_test, y_test):
    rf = RandomForestRegressor(n_jobs=-1, n_estimators= 1000, oob_score = True, bootstrap = True, max_depth=100) # TUNED
...
for method in methods:
    ...
    if method == 'LASSO':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_elastic_cn,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_elastic_cn,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))   
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_rf_cn,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_rf_cn,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
