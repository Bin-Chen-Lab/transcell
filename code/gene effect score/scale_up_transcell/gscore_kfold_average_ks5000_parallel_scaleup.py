...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  4 12:26:39 2020
...
#### save 2000 gscore name
#gscore_name = random.sample(effect_names, 2000)
#gscore_name_df = pd.DataFrame({'gscore_name': pd.Series(gscore_name)})
#gscore_name_df.to_csv('/Users/shanjuyeh/Desktop/Project/GeneExp_prediction/code/gene_effect_score/parallel/scale_up/gscore_name.csv')
...
def default_init_evaluate_model(x_train, y_train, x_test, y_test):
    ...
    new_model.add(LeakyReLU(alpha=0.1))
    new_model.add(Dense(units=288, input_dim=x_train.shape[1], activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
    new_model.add(Dropout(0.6))
    new_model.add(Dense(units=128,activation='tanh', activity_regularizer=regularizers.l2(1e-4), bias_regularizer=regularizers.l2(1e-5)))
    new_model.add(Dropout(0.35))
...
for method in methods:
    ...
    if method == 'default_init':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_default_gscore,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_default_gscore,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
    ...
    if method == 'PCA':
        ...
        p_avg = list(np.mean(res, axis=0))
        p_std = list(np.std(res, axis=0))
        print("%s,avg_pca_gscore,%f,%f,%f,%e,%f,%e,%f\n" % (name, p_avg[0], p_avg[1], p_avg[2],p_avg[3], p_avg[4], p_avg[5], p_avg[6]))
        print("%s,std_pca_gscore,%f,%f,%f,%f,%f,%f,%f\n" % (name, p_std[0], p_std[1], p_std[2],p_std[3], p_std[4], p_std[5], p_std[6]))
